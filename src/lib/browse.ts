...
export function browseLede(b: Browse, locale: Locale): string {
  const fr = locale === 'fr';
  switch (b.kind) {
    case 'tool': return fr ? `Des apps dont ${b.value} est l'outil principal, qui n'ont pas encore gagné un centime (ou tout juste leur premier euro). Classées par jours de travail lus dans le repo GitHub. Pas de revenu, pas de vote : des commits.` : `Apps built mainly with ${b.value} that haven't made a cent yet (or just their first euro). Ranked by days of work read from the GitHub repo. No revenue, no votes: commits.`;
    case 'platform': return fr ? `Des apps ${b.value} faites par une personne, pas encore rentables, classées par jours actifs vérifiés sur GitHub.` : `${b.value} apps made by one person, not profitable yet, ranked by active days verified on GitHub.`;
    case 'language': return fr ? `Des apps écrites en ${b.value}, pas encore rentables, classées par jours actifs lus dans le repo.` : `Apps written in ${b.value}, not profitable yet, ranked by active days read from the repo.`;
