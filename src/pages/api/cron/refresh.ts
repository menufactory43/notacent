...
export const GET: APIRoute = async ({ request }) => {
  ...
  const apps = (await sql.query(
    `select a.id, a.full_name, a.active_days, a.commits, a.status, a.private, a.platform, a.language, a.name, a.description, a.homepage, a.active_dates, u.installation_id
  ...
  for (const a of apps) {
    try {
      ...
      await sql.query(
        `update apps set first_commit=$2, last_commit=$3, commits=$4, active_days=$5, active_days_30=$6, best_streak_weeks=$7, weekly=$8, status=$9, refreshed_at=now(), stars=coalesce($10, stars), platform=$11, active_dates=$12 where id=$1`,
        [a.id, m.first_commit, m.last_commit, m.commits, m.active_days, m.active_days_30, m.best_streak_weeks, m.weekly, status, stars, platform, read.ledger],
      );
